...
export const NetworkPage = () => {
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({
      ...
      layout: { name: 'cose', animate: true, animationDuration: 500, padding: 40 }
    });
  ...
  return (
    <div className="h-[84vh] flex flex-col space-y-4 relative overflow-hidden">
      ...
      {!caseId ? <EmptyState title="Select a case" message="Choose a backend case to load its graph network." /> : <div className="flex-1 relative rounded-2xl border border-purple-500/30 overflow-hidden bg-[#05040A] flex">
        ...
        {network.nodes.length === 0 ? <div className="w-full"><EmptyState title="No graph elements" message="The backend returned no network nodes for this case." /></div> : <div ref={containerRef} className="w-full h-full" />}
        <div className="absolute bottom-4 left-4 right-4 z-20 glass-panel p-2.5 flex justify-between text-[11px] font-mono-id"><span className="text-purple-400 font-bold uppercase">Backend graph</span><span className="text-purple-400/60">{network.nodes.length} nodes · {network.edges.length} edges</span></div>
      </div>}
